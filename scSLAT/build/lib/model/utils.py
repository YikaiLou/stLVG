...
from .graphmodel import LGCN, LGCN_m, LGCN_mlp, LGCN_mlp2, WDiscriminator, ReconDNN
...
def run_SLAT_m(features:List,
    ...
    LGCN_model = LGCN_m(feature_size, hidden_size, K=LGCN_layer, hidden_size=mlp_hidden).to(device)
    optimizer_LGCN = torch.optim.Adam(LGCN_model.parameters(), lr=lr_mlp, weight_decay=5e-4)
...
def run_SLAT_mlp2(features:List,
    ...
    LGCN_model = LGCN_mlp2(feature_size, hidden_size, K=LGCN_layer, hidden_size=mlp_hidden).to(device)
    optimizer_LGCN = torch.optim.Adam(LGCN_model.parameters(), lr=lr_mlp, weight_decay=5e-4)
