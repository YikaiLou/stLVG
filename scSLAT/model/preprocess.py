...
def Cal_Spatial_Net(adata:AnnData,
                    ...
                    verbose:Optional[bool]=True#是否返回处理后的data，是否打印
    ) -> None:
    r"""
    ...
    if model == 'KNN':
        ...
        edge_index = to_undirected(edge_index, num_nodes=adata.shape[0]) # ensure the graph is undirected，确保是无向图
    elif model == 'Radius':
    ...
    #将 PyG 中的边索引数据（存储在 edge_index 中）转换为 pandas DataFrame
...
def scanpy_workflow(adata:AnnData,
    ...
    sc.pp.log1p(adata)#数据标准化和对数化
    sc.pp.scale(adata, max_value=10)
    if n_comps > 0:
        sc.tl.pca(adata, n_comps=n_comps, svd_solver="arpack")#pca降维
