...
from .graphmodel import LGCN, LGCN_AGF_norm, LGCN_mlp, LGCN_mlp2, LGCN_mlp_AGF, WDiscriminator, ReconDNN, DimensionalityReductionModel, AttentionWeighted, Contrast
...
def run_SLAT_mlp2(features:List,
            edges:List,
            coordinates :List,
            epochs:Optional[int]=6,
            LGCN_layer:Optional[int]=1,
    ...
    LGCN_model = LGCN_mlp2(feature_size, hidden_size, coor, K=LGCN_layer, hidden_size=mlp_hidden).to(device)
    optimizer_LGCN = torch.optim.Adam(LGCN_model.parameters(), lr=lr_mlp, weight_decay=5e-4)
    ...
    for i in range(1, epochs + 1):
        ...
        embd0 = LGCN_model(features[0], edges[0], coordinates[0])
        embd1 = LGCN_model(features[1], edges[1], coordinates[1])
...
def run_SLAT_AGF_con(features: List,
                     edges: List,
                     coordinates: List,
                     epochs: Optional[int] = 6,
                     LGCN_layer: Optional[int] = 1,
    ...
    time2 = time.time()
    return embd0_0, embd0_1, embd1_0, embd1_1, embd0, embd1, time2 - time1
...
def run_SLAT_mlp_AGF(features: List,
    ...
    def train_stage_1(model, optimizer, wdiscriminator, optimizer_wd, recon_model, optimizer_recon, m):
        ...
        for epoch in range(1, epochs + 1):
            ...
            # print(f'Epoch [{epoch}/{epochs}], Loss GAN: {loss_gan.item()}, Loss Recon: {loss_recon.item()}, Total Loss: {loss.item()}')
    ...
    for epoch in range(epochs + 1):
        ...
        # embd0_reduced = weighted_embd0
        # embd1_reduced = weighted_embd1
        ...
        # loss_combined = 0.1 * loss_gan_combined + 0.9 * loss_recon_combined
        # total_loss = (1 - 0.8) * loss_gan_combined + 0.8 * loss_recon_combined
        ...
        # print(f'Contrastive Loss 0: {contrastive_loss_0.item()}')
        # print(f'Contrastive Loss 1: {contrastive_loss_1.item()}')
        ...
        # print(f'Epoch {epoch}, w0: {w0.item()}, w1: {w1.item()}')
    ...
    time2 = time.time()
    # print(f'Attention weights for m=0: {lamda_0.cpu().detach().numpy()}')
    # print(f'Attention weights for m=1: {lamda_1.cpu().detach().numpy()}')
    # print('Training model time: %.2f' % (time2 - time1))
...
def run_SLAT_AGF_contrast(features: List,
    ...
    while True:  # Infinite loop until break condition is met
        ...
        contrastive_loss = contrastive_loss_0 + contrastive_loss_1
        weighted_embd0 = torch.concat([embd0_proj_0, embd0_proj_1], dim=1)
        weighted_embd1 = torch.concat([embd1_proj_0, embd1_proj_1], dim=1) #可注释
