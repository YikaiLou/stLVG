...
def train_GAN(wdiscriminator:torch.nn.Module,
                optimizer_d:torch.optim.Optimizer,#优化判别器的参数
                embds:List[torch.Tensor],#包含两个张量的列表，分别表示两个图的节点嵌入
                batch_d_per_iter:Optional[int]=5,#每次训练迭代中判别器的批量训练次数
                anchor_scale:Optional[float]=0.8  #锚定细胞的比例
    )->torch.Tensor:
    r"""
    ...
    return loss
#尽可能准确地区分来自真实数据分布和生成器产生的数据分布的样本，判别器的训练目的是使其能够有效地区分这两种分布，并输出合适的损失
...
#训练重构模型，重构模型的目标是尽可能准确地将输入的节点嵌入（embds）重构为原始的节点特征（features）
...
#根据余弦相似度去判断embedding后与真实情况的对应关系，并根据相似度排序获取了每个节点的最近邻
#根据地面真实情况中的映射关系，计算了在不同的top k值下，嵌入之间的对应关系的准确率。这里的准确率指的是，在给定的top k个最近邻中，有多少是正确的